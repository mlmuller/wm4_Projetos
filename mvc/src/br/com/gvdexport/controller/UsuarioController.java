  ...
  //  	Usuario aux = genericoDao.find(usuario.getUsuarioid());
	...
	public void validaUsuario() throws IOException {
		...
		if (!usuarios.isEmpty()) {
			...
			Boolean ok;
			if(aux == null) {
				Messages.addGlobalError("Dados fornecidos não conferem,Verifique!");
...
//					this.usuarioLogado.setUsuariologado(aux);
			...
			if(!Nome.equals("mauro") || (!Senha.equals("355193"))) {
				context.addMessage(null, new FacesMessage("Você não tem autorização para inicializar o SISTEMA!"));
				Nome="";
				Senha="";
				return;
			}
