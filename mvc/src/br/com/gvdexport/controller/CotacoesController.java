...
import br.com.gvdexport.util.TimeLookUp;
...
public class CotacoesController implements Serializable {
	...
	public void save() {
		try {
	        ...
	        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,"Nï¿½o foi possivel,executar Processo!",""));
			ex.printStackTrace();
		}
	...
	public void delete(Cotacao cotacao) {
		...
		try {
			...
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,"Nï¿½o foi possivel Cancelar Cotaï¿½ï¿½o!",""));
			ex.printStackTrace();
		}
	...
	public void localizaCotacao() {
		...
		//moeda,mes,ano
		//Se intervalo selecionado,nao seleciona mes e ano
		if((dataInicial != null) && (dataFinal == null)) {
		   dataFinal = dataInicial;	
		}
		if((dataInicial == null) && (dataFinal != null)) {
			Messages.addGlobalInfo("Selecionde Data Inicial !", "");
			return;
		}
		if ((dataInicial == null) && (dataFinal == null)) {
			if ((cotacao.getMoedas() != null)||(cotacao.getMes()!= null)||(cotacao.getAno() != null)) {
				
				if(((!cotacao.getMoedas().equals(null)) && (cotacao.getAno() != null)) && (cotacao.getMes() == null)) {
					Messages.addGlobalInfo("Moeda Selecionada! Ano selecionado! Selecione tambï¿½m Mï¿½s!", "");
					return;
				}
				if((cotacao.getMoedas() == null) && ((cotacao.getAno() != 0) || (cotacao.getMes() == null))) {
					Messages.addGlobalInfo("Ano selecionado,Moeda/Mï¿½s selecionado,Selecione Moeda!", "");
					return;
				}
				if((cotacao.getMoedas() !=null) && ((cotacao.getAno() == null) || (cotacao.getMes() == null))) {
					Messages.addGlobalInfo("Moeda Selecionada !,selecione tambï¿½m o Ano/Mï¿½s!", "");
					return;
				}
			}else {
				if((cotacao.getAno()== null)&&(cotacao.getMes()==null)&&(cotacao.getMoedas() == null)) {
				  refresh();
				  return;
				}
			}
		}else {
			if(((cotacao.getAno()!=null) || (cotacao.getMes()!=null)) && ((dataInicial != null) ||(dataFinal != null))) {
				Messages.addGlobalWarn("Selecionando Intervalo!,Nï¿½O selecione ANO/Mï¿½S !", "");
				return;
			}
			if(((dataInicial!=null) && (dataFinal != null)) && (cotacao.getMoedas() == null)) {
				Messages.addGlobalWarn("Selecionou intervalo! Selecione Moeda Tambï¿½m !","");
				return;
			}
		}
		listaCotacoes =  new ArrayList<>();
		Calendar auxCal = Calendar.getInstance();
		auxCal.setTime(dataInicial);
		java.sql.Date inicial = new java.sql.Date(auxCal.getTimeInMillis());
		auxCal.setTime(dataFinal);
		java.sql.Date fim = new java.sql.Date(auxCal.getTimeInMillis());
		listaCotacoes=facadeCompKey.getBuscaTaxa(cotacao.getMoedas().getSigla().trim(), cotacao.getAno(), cotacao.getMes(), inicial, fim);
		if(listaCotacoes.size() == 0) {
			Messages.addGlobalWarn("Cotaï¿½ï¿½es nï¿½o encontradas ! Verifique informaï¿½ï¿½es passadas na consulta!", "");
	...
	public void preparaAdicaoCorrente() {
		...
		if (mAno == null) {
			Messages.addGlobalWarn("Por favor,ajuste imediatamente data do seu computador!", "");
			mAno = 0;
			return;
		}
	...
	public void encontraUltimoMes() {
		...
		if(((cotacaomesnovo.getAno().equals(null)) || (cotacaomesnovo.getAno().equals(0))) || 
			...
			Messages.addGlobalInfo("Informe Moeda Geração do Próximo Mês!","");
			return;
		}
		mNumeral = 0;
		for (int i = 0; i < listaMoedas.size(); i++) {
...
//	@SuppressWarnings({ "static-access", "unchecked" })
	public void preparaVetorAtualizacao() throws ParseException, CloneNotSupportedException {
        ...
        if ((mNumeral == 1)||(mNumeral == 3)||(mNumeral == 5)||(mNumeral == 7)||(mNumeral == 8)||(mNumeral == 10)||(mNumeral == 12)) {
			...
			for (int j = 1; j < (DiasdoMes+1) ; j++) {
				...
				auxDate= auxCal.getTime();
				if((auxCal.DAY_OF_WEEK > 1) && (auxCal.DAY_OF_WEEK < 7)) {
					auxCotacao.setDatacotacao(auxDate);
				}
			}
        }
        ...
        try {
        	...
        	if (listaAuxPeriodo.size() > 0 ) {
        		...
        		cotacaoEspecialDao.saveVetorCotacao(listaAuxPeriodo);
    			context.addMessage("",new FacesMessage("Cotaï¿½ï¿½es inseridas com Sucesso!"));
        	}
	        refresh();
		} catch (RuntimeException ex) {
			Messages.addGlobalWarn("Nï¿½o foi possivel inserir mes solicitado!","");
			ex.printStackTrace();
		}
	}
    ...
    public void upload(FileUploadEvent evento)  {
		try {
			arquivo = evento.getFile();
			Path arquivoTemp = Files.createTempFile(null, null);
			Files.copy(arquivoTemp, null);
...
//				@SuppressWarnings("resource")
				...
				if (!leitor.hasNext()){
					Messages.addGlobalError("Arquivo a importar CSV,estï¿½ vazio!", "");
					leitor.close();
					return;
				}
				encontraUltimoMes();
				while (leitor.hasNext()) {
					try {
					  ...
					  if(umavez == 0) {
						  if((cotacaomesnovo.getMoedas().getSigla().substring(0, comprimentoString).equals(sigla))) {
							 umavez = 1; 
							  ...
							  Messages.addGlobalError("Moeda Selecionada nï¿½o ï¿½ a mesma do arquivo a importar!","");
	  						  leitor.close();
							  return;
						  }
					  }
					  ...
					  if ((mesExistente > mNumeral) && (mesExistente-mNumeral)> 1)  {
						  Messages.addGlobalError("Moeda/Mï¿½s,arquivo a importar, pode ter um Mï¿½s de cada vez,Verifique Demonstrativo!", "");
	 					  leitor.close();
						  return;
					  }
					  //ok
					  ...
					  if(!cotacaomesnovo.getAno().equals(Integer.parseInt(dataAuxano))) {
						  Messages.addGlobalError("Ano/Moeda Selecionado nï¿½o ï¿½ a mesmo do arquivo a importar!","");
						  leitor.close();
						  return;
					  }
					  String auxData=(dataAuxano+"-"+dataAuxMes+"-"+dataAuxDia);
					  ...
					  if (sigla == "EURO") {
						  ...
						  aux.setVlrcomprareais(vlr3);
						  aux.setVlrvendareais(vlr4);
						  aux.setValorcompra(vlr1);
						  aux.setValorvenda(vlr2);
					  }
					  ...
					  aux=new Cotacao();
						...
						Messages.addGlobalError("Nï¿½o foi possivel criar Lista definitiva da moeda Solicitada!", "");
						return;
					} catch (ParseException ex) {
						Messages.addGlobalError("Erro na conversï¿½o de datas do arquivo CSV!", "");
						ex.printStackTrace();
						leitor.close();
						return;
					}
	        ...
	        if (mNumeral > mesExistente) {
			    Messages.addGlobalInfo("Moeda/Mï¿½s Selecionado jï¿½ estï¿½o Cadastrados!","");
			    Messages.addGlobalInfo("Verificando Mï¿½s e se necessario serï¿½ completado !", "");
			    refresh();
			    Messages.addGlobalInfo("Fim Verificaï¿½ï¿½o!", "");
			    return;
	        }
	        Messages.addGlobalInfo("Cadastro Taxas realizado com Sucesso !", "");
	        refresh();

		} catch (IOException | ParseException ex) {
			Messages.addGlobalInfo("Não foi possivel criar Arquivo Temporï¿½rio !");
			ex.printStackTrace();
			return;
		}
	}
